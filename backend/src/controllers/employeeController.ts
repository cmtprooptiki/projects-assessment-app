...
export const getAll = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { department, isActive, isExternal, search, page, limit, sortBy, sortOrder } = req.query as Record<string, string>;
    const result = await employeeService.getAllEmployees({
      department, isActive, isExternal, search, sortBy, sortOrder,
